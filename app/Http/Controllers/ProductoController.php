...
class ProductoController extends Controller
{
    ...
    {
        // Obtiene todas las instancias de la clase "Cliente" ordenadas por el campo "nombre"
        $clientes = Cliente::orderBy('nombre')->get();
        //si no existen clientes, se redirige a la vista de creación de clientes
        if ($clientes->isEmpty()) {
            return redirect()->route('clientes.create')->with('info', 'Primero debes crear un cliente');
        }
        return view('productos.create', ['clientes' => $clientes]);
    }
    ...
    {
        $clientes = Cliente::orderBy('nombre')->get();
        return view('productos.edit', ['producto' => $producto, 'clientes' => $clientes]);
    }
